...
def _crawl_alumni_profiles(**kwargs):
    ...
    storage_client = storage.Client.from_service_account_json(
        service_account_key_file)
    stats = file_exists_on_gcs(
        storage_client,
        bucket_name,
        f"{profiles_folder_path}Cohort_" + str(cohort_id) + ".json",
    )
    ...
    if stats:
        ...
        write_csv_to_gcs(
            storage_client,
            bucket_name,
            f"invalid_profiles/Cohort_" + str(cohort_id) + ".csv",
            invalid_urls,
        )
...
with DAG(
    ...
    for cohort, profiles in get_cohorts().items():
        ...
        crawl_alumni_profiles = PythonOperator(
            task_id=f"crawl_alumni_profiles_cohort_{cohort}",
