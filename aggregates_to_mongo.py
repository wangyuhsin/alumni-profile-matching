...
### UDF Functions ###
udf_volunteer_titles = udf(lambda x: _params(
    x, "title"), ArrayType(StringType()))
...
def insert_aggregates_to_mongo(profiles_path_by_cohort):
    ...
    df = (
        alumni_data.withColumn("languages", concat_ws(" ", col("languages")))
        ...
        .select(
            ...
            "volunteer_work",
            col("exp")[0].alias("titles"),
            col("exp")[1].alias("companies"),
            col("exp")[2].alias("total_years_of_experience"),
            col("exp")[3].alias("exp_descriptions"),
            "certifications",
        )
    )

    mongodb = MongoDBCollection(
        mongo_username, mongo_password, mongo_ip_address, database_name, collection_name
    )
    columns = df.columns
    rdd = df.rdd
    aggregates = rdd.map(lambda x: dict(zip(columns, x)))
    for aggregate in aggregates.collect():
        aggregate["cohort"] = profiles_path_by_cohort.split(
            "/")[-1].split(".")[0]
